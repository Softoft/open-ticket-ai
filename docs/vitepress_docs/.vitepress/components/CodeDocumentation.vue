  ...
  <div v-else-if="classData || packageData" class="code-documentation-wrapper card shadow-sm mb-4">
    ...
    <div v-if="classData" class="card-body">
      ...
      <div v-if="methodsToDisplay.length > 0" class="mt-4">
        <h3>Methods</h3>
        <div class="accordion" :id="`accordion-methods-${classId}`">
          <FunctionDoc v-for="method in methodsToDisplay" :key="method.name" :func="method" :parent-id="classId" />
        </div>
      </div>
    </div>
    ...
    <div v-if="packageData" class="card-body">
      ...
      <div v-if="showAllFunctions && packageData.functions.length > 0" class="mt-4">
        <h4>Functions</h4>
         <div class="accordion" :id="`accordion-functions-${packageId}`">
          <FunctionDoc v-for="func in packageData.functions" :key="func.name" :func="func" :parent-id="packageId" />
        </div>
      </div>
    </div>
  </div>
...
const subPackagesToDisplay = computed<ModuleEntry[]>(() => {
