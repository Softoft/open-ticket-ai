...
function genericArray(a3, b) {
  ...
  for (i = 0; i < na; ++i) x2[i] = value_default(a3[i], b[i]);
  for (; i < nb; ++i) c5[i] = b[i];
  return function(t) {
    for (i = 0; i < na; ++i) c5[i] = x2[i](t);
    return c5;
  };
}
...
function centroid_default(object2) {
  ...
  if (m2 < epsilon22) {
    x2 = X1, y2 = Y1, z = Z1;
    if (W1 < epsilon7) x2 = X0, y2 = Y0, z = Z0;
    m2 = hypot(x2, y2, z);
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x2, y2) {
    if (f > 0) {
      if (y2 < -halfPi3 + epsilon7) y2 = -halfPi3 + epsilon7;
    } else {
      if (y2 > halfPi3 - epsilon7) y2 = halfPi3 - epsilon7;
    }
