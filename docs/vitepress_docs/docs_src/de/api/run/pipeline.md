...
- **`data`** (`dict[str, Any]`) (Standard: `an empty dictionary`) - Ein Dictionary zur Speicherung beliebiger Daten für Pipeline-Stufen. Standardmäßig ein leeres Dictionary.
...
Zusammengesetztes Pipe, das eine Sequenz von Pipes ausführt.
...
- **`pipes`** () - Geordnete Liste der sequenziell auszuführenden Pipe-Instanzen.
...
**Rückgabe:** () - Der finale Kontext nach Verarbeitung durch alle Pipes.
...
**Rückgabe:** () - Der modifizierte Kontext nach Pipeline-Ausführung.
